...
const SearchPage = () => {
  ...
  const availableTimes = useMemo(() => [{ label: `12:00 AM`, value: `12:00 AM` }].concat(
  ...
  const submitSearch = () => {
    const searchParams = {
      ...
      instructorFirstName: instructorFirstName.length > 0 ? instructorFirstName : "",
      instructorLastName: instructorLastName.length > 0 ? instructorLastName : "",
  ...
  const availableTerms = useMemo(() => generate_available_terms(termType), [termType]);
  ...
  const courseAttributeOptions = useMemo(() => (
    searchOptions.crse_attrs?.flatMap((attributeGroup) => {
      if (Array.isArray(attributeGroup.values) && attributeGroup.values.length > 0) {
  ...
  const handleSubjectChange = React.useCallback((value: string[]) => setSubject(value), []);
  const handleCourseAttributesChange = React.useCallback((value: string[]) => setCourseAttributes(value), []);
  ...
  const handleSearchTermChange = React.useCallback((value: string[]) => setSearchTerm(value), []);
  const handleNumberOfUnitsChange = React.useCallback((value: string[]) => setNumberOfUnits(value), []);
  const handleStartTimeChange = React.useCallback((value: string[]) => setStartTime(value), []);
  const handleEndTimeChange = React.useCallback((value: string[]) => setEndTime(value), []);
  const handleInstructModeChange = React.useCallback((value: string[]) => setInstructMode(value), []);
  const handleInstructorFirstNameChange = React.useCallback((e: React.ChangeEvent<HTMLInputElement>) => setInstructorFirstName(e.target.value), []);
  const handleInstructorLastNameChange = React.useCallback((e: React.ChangeEvent<HTMLInputElement>) => setInstructorLastName(e.target.value), []);
  const handleInstructorScoreChange = React.useCallback((e: React.ChangeEvent<HTMLInputElement>) => setInstructorScore(e.target.value), []);
      ...
      >
        ...
        <div style={{ flex: "0 0 auto" }}>
          <SearchOptSelector
            ...
            label="Course Attributes" multiple={true}          />
        </div>
        <div style={{ flex: "0 0 auto" }}>
          <SearchOptSelector
            ...
            label="Day of the Week" multiple={dayOfTheWeek.includes("any") ||  dayOfTheWeek.includes("MTWTF") ? false : true}          />
        </div>
        <div style={{ flex: "0 0 auto" }}>
          <SearchOptSelector
