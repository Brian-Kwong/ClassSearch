    ...
    parentPort.on('message', async (message) => {
        if (message.type === 'loadModel') {
            if(searchPipeline && lookupTable) {
                ...
                // @ts-ignore
                searchPipeline = await pipeline('feature-extraction', 'sentence-transformers/all-MiniLM-L6-v2') as FeatureExtractionPipeline;
        ...
        } else if (message.type === 'semanticSearch' && searchPipeline && lookupTable) {
            const courses : { subject_descr: string }[] = message.courses;
            const searchResults : { lib: string; name: string }[] = [];
            for(const course of courses) {
                const embeddings = await searchPipeline(course.subject_descr, { pooling: 'mean', normalize: true });
                if(!embeddings || (Array.isArray(embeddings) && embeddings.length === 0)) {
                    searchResults.push({ lib: 'mdi', name: 'book' });
                    continue;
                } else {
                    const results = await lookupTable.search(Array.from(embeddings.data)).limit(1).execute();
                    if (results.length > 0) {
            ...
            parentPort?.postMessage({ type: 'semanticSearchResults', results: searchResults });
        }
    });
}